...
// Enhanced TUI function to display songs in a table with album art
void display_songs_table_with_art(cJSON *batch, struct notcurses *nc) {
        ...
        if (artist != NULL && track_name != NULL) {
          ...
          if (images && cJSON_GetArraySize(images) > 0) {
            ...
            if (url && url->valuestring) {
              // In a real implementation, you would download and display the image here
              // For now, just indicate that album art is available
              char art_indicator[100];
              sprintf(art_indicator, "║ 🖼️  Album art available: %s", url->valuestring);
  ...
  ncplane_putstr_yx(n, row++, 0, "║                                                                                              ║");
  ncplane_putstr_yx(n, row++, 0, "╚═══════════════════════════════════════════════════════════════════════════════════════════════════╝");
  ncplane_putstr_yx(n, row + 1, 0, "Press 'q' to quit. Use arrow keys to scroll (not implemented yet)");
  notcurses_render(nc);
}
...
int main(const int argc, char *argv[]) {
  ...
  if (parsed_json_response == NULL) {
    ...
    if (liked_songs) {
      // Display songs using enhanced TUI with table and album art
      display_songs_table_with_art(liked_songs, nc);
